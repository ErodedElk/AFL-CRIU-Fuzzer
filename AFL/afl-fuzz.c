...
int afl_fuzzer(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "aji:o:f:m:b:t:T:dnCB:S:M:x:QVD:")) > 0)

    switch (opt) {
      case 'a': /*afl mode open*/
        break;
      case 'D': /*criu image path*/
        break;
      case 'j': /*criu  shell job */
        break;
