...
int main(int argc, char *argv[], char *envp[])
{
	...
	init_opts();
	_backup_argv=(char**)malloc((argc)*sizeof(char*));
	memset(_backup_argv, 0, (argc)*sizeof(char*));
	_backup_argv[0]=strdup(argv[0]);
	for(int i=2; i<argc; i++)
	{
		_backup_argv[i-1]=strdup(argv[i]);
	}
	...
	if (opts.mode == CR_RESTORE) {
		...
		if(opts.afl_mode)
		{
			afl_fuzzer(argc-1,_backup_argv);
		}
