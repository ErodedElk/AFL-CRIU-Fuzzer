...
static int finalize_restore_detach(void)
{
	...
	for_each_pstree_item(item) {
		...
		for (i = 0; i < item->nr_threads; i++) {
			...
			{
				pr_perror("Unable to get Register %d", pid);
				return -1;
			}

			data = ptrace(PTRACE_PEEKDATA, pid,regs.rip, NULL);
			if(data!=0 && (data&0xffff)==0xfeeb)
			{
				code =(data&0xffffffffffff0000)+0x9090;
				if(ptrace(PTRACE_POKEDATA,pid,regs.rip,code))
				{
					pr_perror("Unable to recover byte %d", pid);
					return -1;
				}
			}
